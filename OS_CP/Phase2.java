...
public class Phase2 {
    ...
    static void LOAD() throws IOException {
        while (fin.ready()) { //reads from start to end of input file
            buffer = fin.readLine();
    ...
    static int ADDRESS_MAP(int VA) {
        if (VA >= 0 && VA <= 99) { // valid operand (0 to 99)
            ...
            PI = 2; // invalid operand (operand errror) . 
            return -1;
        }
    }
    ...
    static int ALLOCATE() {
        ...
        return x; // allocate new frame number 0-29
    }
    ...
    static void READ() throws IOException {
        ...
        if (buffer != null) {
            if (buffer.startsWith("$END")) { // out of data
                fin.reset();// no data if found in data card thus we have to termanate the program!
    ...
    static void WRITE() throws IOException {
        PCB.LLC++; // inc LLC  total no. of output lines in given job
        if (PCB.LLC > PCB.TLL) {
